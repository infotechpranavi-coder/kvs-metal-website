...
export { SESSION_COOKIE, SUPERADMIN_COOKIE }
...
function verifySignedToken(token: string | undefined, prefix: string): boolean {
...
export function verifySuperAdminSessionToken(token: string | undefined): boolean {
  return verifySignedToken(token, 'superadmin:')
}
...
/** Admin or superadmin — used for CMS edits and uploads. */
export async function isCmsEditorAuthenticated() {
  return (await isDashboardAuthenticated()) || (await isSuperAdminAuthenticated())
}
...
export function superAdminCookieOptions(token: string) {
  return {
...
export function clearSuperAdminCookieOptions() {
  return {
    name: SUPERADMIN_COOKIE,
    value: '',
    path: '/',
...
export function checkSuperAdminCredentials(username: string, password: string) {
  const expectedUser = process.env.SUPERADMIN_USERNAME || 'superadmin'
  const expectedPassword = process.env.SUPERADMIN_PASSWORD || 'superadmin123'
  return username === expectedUser && password === expectedPassword
}
